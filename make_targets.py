...
def make_targets(symbol, start_date, end_date, price_data_sp500):
    '''
...
def make_targets_all_symbols(start_date, end_date):
    '''
    ...
    for i, symbol in enumerate(symbol_names):
        df = make_targets(symbol, start_date, end_date, price_data_sp500)
...
def main():
    ...
    end_date='2023-12-31'
    targets_df = make_targets_all_symbols(start_date, end_date)
    ...
    targets_df_filtered['era'] = np.empty(len(targets_df_filtered))
    # for i in range(len(targets_df_filtered)):
    #     targets_df_filtered.loc[i, 'era'] = targets_df_filtered.loc[i, 'report_date'].year
    # targets_df_filtered['era'] = targets_df_filtered['report_date'].apply(lambda x: x.year)
    # print(targets_df_filtered['era'])
    # print(targets_df_filtered.columns())
    targets_df_filtered['era'] = targets_df_filtered['report_date'].dt.year
    # print(targets_df_filtered['era'])
    ...
    targets_df_filtered.set_index(['era', 'symbol'], inplace=True)
    targets_df_filtered_dedup = targets_df_filtered.drop_duplicates(inplace=False).reset_index()
